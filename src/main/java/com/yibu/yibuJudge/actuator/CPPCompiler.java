...
public class CPPCompiler implements Compiler {
    ...
    public CPPCompiler(ExecProperties execProperties,
    ...
    public String build(String codeSource, boolean rmSource) throws CompilerException {
        ...
        String[] command = {execProperties.getGpp(),realCodePath, "-std=c++11", "-o", fileProperties.getBuildPath() + "/" + buildDir};
        try {
    ...
    public void deStory(String buildDir) {
        ...
        if (!del) {
            System.out.println("delete build file error " + buildPath);
    ...
    public String saveCode(String code) {
        String codeSource = UUID.randomUUID().toString().replace("-", "") + ".cpp";
