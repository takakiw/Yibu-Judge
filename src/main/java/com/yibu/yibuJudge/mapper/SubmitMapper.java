...
public interface SubmitMapper {
    ...
    @Select("SELECT s.id, s.status, s.runtime,s.submit_time, l.name as languageName " +
    ...
    @Select("select date_format(submit_time, '%Y-%m-%d') as submitTime, count(*) as submitCount from submit where user_id = #{uid} and year(submit_time) = #{year} group by date_format(submit_time, '%Y-%m-%d')")
    ...
    @Update("update submit set status = #{status}, runtime = #{runtime}, memory = #{memory}, result_message = #{resultMessage}, code_path = #{codePath} where id = #{submitId}")
