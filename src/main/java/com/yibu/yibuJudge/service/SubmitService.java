...
public class SubmitService {
    ...
    public void updateSubmitResult(Long submitId, JudgeResult judgeResult) {
        ...
        if (submit.getContestId() != null && statusCode == SubmitStatusCode.ACCEPTED){ // 竞赛题目, 只有ac才更新排名
            ...
            if (status.getAcCount() > 0) // 已经有过ac， 不需要更新排名
            {
                return;
            }
            // 获取比赛信息
            ...
            log.info("用户{}在比赛{}中提交题目{}，提交时间{}， 耗时{}， 罚时{}", submit.getUserId(), submit.getContestId(), submit.getProblemId(), submit.getSubmitTime(), time, penaltyTime);
            ContestProblems contestProblems = contestMapper.getContestProblem(submit.getContestId(), submit.getProblemId());
