...
public class ProblemService {
    ...
    public Page<ProblemPage> getProblemList(int page, int size, List<String> tags, String title, Integer difficulty, String order, String sort) {
        ...
        if (uid != null && !idList.isEmpty()) {
            Map<Integer, SubmitStatus> submitStatus = submitService.submitStatus(idList, uid);
            for (ProblemPage problem : problemList) {
                SubmitStatus submitStatu = submitStatus.getOrDefault(problem.getId(), new SubmitStatus());
                problem.setStatus(submitStatu == null ? 0 : SubmitStatusUtil.getProblemStatus(submitStatu.getAcCount(), submitStatu.getTotalSubmissions()));
            }
        }
        return problemList;
    }
    ...
    public void updateProblem(Integer id, String title, String description, String inputDesc,
        ...
        if (StringUtils.isNotBlank(title)) {
            dbProblem.setTitle(title);
        }
        if (StringUtils.isNotBlank(description)) {
            dbProblem.setDescription(description);
        }
        if (StringUtils.isNotBlank(inputDesc)) {
            dbProblem.setInputDesc(inputDesc);
        }
        if (StringUtils.isNotBlank(outputDesc)) {
            dbProblem.setOutputDesc(outputDesc);
        }
        if (timeLimit != null) {
            dbProblem.setTimeLimit(timeLimit);
        }
        if (memoryLimit != null) {
            dbProblem.setMemoryLimit(memoryLimit);
        }
        if (difficulty != null) {
            dbProblem.setDifficulty(difficulty);
        }
        if (auth != null) {
            dbProblem.setAuth(auth);
        }
