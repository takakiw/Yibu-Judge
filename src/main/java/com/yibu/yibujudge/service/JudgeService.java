            ...
            if (result != Result.SUCCESS) {
                ...
                try {
                    if (result != Result.RUNTIME_ERROR) {
                        ...
                        judgeResult.setOutput(IoUtil.readUtf8(new FileInputStream(commandArgs.getRealErrorPath(0))));
                    }
                } catch (Exception e) {
        ...
        try {
            ...
            for (int i = 0; i < commandResultList.size(); i++) {
                ...
                if (result != Result.SUCCESS) {
                    ...
                    if (result != Result.RUNTIME_ERROR) {
                        judgeResult.setFirstErrorYueOutput(IoUtil.readUtf8(new FileInputStream(commandArgs.getRealOutputPath(0))));
                    } else {
                        judgeResult.setFirstErrorYueOutput(IoUtil.readUtf8(new FileInputStream(commandArgs.getRealErrorPath(0))));
                    }
                    return judgeResult;
                }
                // 读取程序输出
